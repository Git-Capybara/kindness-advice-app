...
class LostStarGame:
    ...
    def start_round(self):
        ...
        self.message_label.config(text="Watch the stars!", fg="yellow")

        new_star_index = random.randint(0, len(self.star_objects) - 1)
        self.sequence.append(new_star_index)

        self.master_frame.after(500, lambda: self.show_sequence(0))

    def show_sequence(self, index):
        if index < len(self.sequence):
            star_id = self.star_objects[self.sequence[index]]
            self.canvas.itemconfig(star_id, fill="gold")
            self.master_frame.after(500, lambda: self.canvas.itemconfig(star_id, fill="gray"))
