...
class LostStarGame:
    ...
    def setup_ui(self):
        ...
        self.message_label = tk.Label(self.master_frame, text="A Starlight Sprite needs your help! Watch the sequence!", font=("Arial", 12, "italic"), fg="yellow", bg="#f0f8ff")
        self.message_label.pack(pady=5)
    ...
    def create_stars(self):
        ...
        for i, (x, y) in enumerate(star_positions):
            star = self.canvas.create_text(x, y, text="✨", font=("Arial", 36), fill="red", tags=f"star_{i}")
    ...
    def start_round(self):
        ...
        self.message_label.config(text="Watch the stars!", fg="blue")
        new_star_index = random.randint(0, len(self.star_objects) - 1)
        self.sequence.append(new_star_index)
        self.master_frame.after(500, self.show_sequence, 0)

    def show_sequence(self, index):
        if index < len(self.sequence):
            star_id = self.star_objects[self.sequence[index]]
            self.canvas.itemconfig(star_id, fill="brown")
            # Schedule turning the star off and moving to the next one
            self.master_frame.after(500, self.turn_off_and_next, star_id, index + 1)
        else:
            self.master_frame.after(500, lambda: self.message_label.config(text="Now you try!", fg="white"))
            self.game_active = True

    def turn_off_and_next(self, star_id, next_index):
        self.canvas.itemconfig(star_id, fill="purple")
        self.master_frame.after(200, self.show_sequence, next_index)
    ...
    def on_star_click(self, event):
        ...
        if star_index != -1:
            ...
            self.master_frame.after(300, lambda: self.canvas.itemconfig(item_id, fill="orange"))
            self.check_sequence()
